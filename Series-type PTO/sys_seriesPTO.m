...
%
% PURPOSE/DESCRIPTION:
% Calculate the state derivatives for a series-type wave energy PTO 
...
%
% UPDATES:
% 1/4/2024 - created from sys_parPTO.m.
%
...
control.qpm_dpdtMin = q_ro - C_ro*par.control.dpdt_ROmax;
control.qpm_dpdtMax = q_ro + C_ro*par.control.dpdt_ROmax;
    % estimate shaft speed
delta_p_pm = y(par.iy.p_ro) - y(par.iy.p_hout);
a = par.APP.C_s*delta_p_pm/par.mu;
b = 1 - delta_p_pm/par.beta*(par.APP.V_r + 1);
control.w_pm_dpdtMin = (control.qpm_dpdtMin/par.D_pm + a)/b;
control.w_pm_dpdtMax = (control.qpm_dpdtMax/par.D_pm + a)/b;
    % enforce limits
...
volLoss_pm = abs(delta_p_pm)*(par.APP.C_s/(par.mu*abs(control.w_pm)) ...
                    + (par.APP.V_r + 1)/par.beta);
nonState.q_pm = (nonState.pmPumping*(1 - volLoss_pm) ...
                + nonState.pmMotoring*(1 + volLoss_pm)) ...
                *par.D_pm*control.w_pm;

mechLoss_pm = par.APP.C_v*par.mu*abs(control.w_pm/delta_p_pm) ...
                    + par.APP.C_f;
nonState.Tpm = (nonState.pmPumping*(1 + mechLoss_pm) ...
