...
titleString = ['Pressure: Parallel-Type with Active Element'];
...
% yyaxis right
% plot(out.t,1e3*out.q_pm(:),'LineStyle','-.','LineWidth',lineWidth)
...
titleString = ['Rate of Change in Feed Pressure: Parallel-Type with Active Element'];
...
% plot(out.t([1 end]),1e-3*dpdt_99*[1 1],'--k','LineWidth',lineWidth)
% p(2) = plot(out.t([1 end]),1e-3*dpdt_99*[-1 -1],'--k','LineWidth',lineWidth);
...
% leg = legend('$dp_{ro}/dt$','$\pm P_{97} |dp_{ro}/dt|$', ...
%              '$\pm P_{99} |dp_{ro}/dt|$','target limit', ...
%              'Interpreter','latex');
leg = legend('$dp_{ro}/dt$','target limit', ...
             'Interpreter','latex');
leg.FontSize = axFontSize;
leg.FontName = 'Times';
leg.Orientation = 'horizontal';
...
titleString = ['Resistive Element Flow Coefficient: Parallel-Type with Active Element'];

semilogy(out.t,max(1e-6,out.control.kv_ideal*sqrt(1000)*1000),'LineWidth',lineWidth,'Color',gold)
hold on
semilogy(out.t,max(1e-6,out.control.kv_rv*sqrt(1000)*1000),'--','LineWidth',lineWidth,'Color',maroon)

xlabel('time (s)', ...
'Interpreter','latex','FontSize',axFontSize,'fontname','Times')
ylabel('coefficent (L/s/kPa$^{1/2}$)', ...
'Interpreter','latex','FontSize',axFontSize,'fontname','Times')
