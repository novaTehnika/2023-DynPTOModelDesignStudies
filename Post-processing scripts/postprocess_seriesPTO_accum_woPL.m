...
files = dir;
nfiles = size(files,1);
dataFileName = "data_seriesPTO_accum";
for j = 1:nfiles
...
titleString = ['Power Loss Normalized to Mean Power Capture With Series-Type PTO:',newline,...
...
for i = 1:N
    iVar = iDom(iDomStart(i):iDomStart(i+1)-1);
    p(iax,i) = plot(XaxisVar(iVar),D_pm_mesh(iVar)/1e-6*(2*pi));
    p(iax,i).Color = [color(i,:)];
    p(iax,i).Marker = markerType(i);
    p(iax,i).MarkerSize = 5;
    p(iax,i).LineWidth = 1.5;
